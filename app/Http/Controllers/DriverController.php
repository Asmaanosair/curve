...
class DriverController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $fleets = $query->where('application_status_id',DriverApply::APPROVED_BY_PUDO)->paginate($request->get('itemsLimit'));
        $onlineCount  = $query->where("status",'online')->count();
    ...
    public function fleetsInDuty(Request $request,$order_id){

        $user = auth('api')->user();
        $order = Order::find($order_id);

        $query = Fleet::withCount('orders');
        ...
        //                 ST_MakePoint($order->pick_up_lng, $order->pick_up_lat)::geography,
        //                 5000)");
